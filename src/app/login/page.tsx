...
export default function Page() {
  ...
  const handleLogin = async () => {
    ...
    try {
      const response = await fetch(`${process.env.PROD_BASE_URL}/api/auth/login`, {
        method: "POST",
        headers: {
  ...
  return (
    ...
    >
      <div className="w-full flex gap-4 flex-1">
        <Link href={`${process.env.PROD_BASE_URL}/api/auth/google`} className="py-3 px-6 gap-4 flex justify-center flex-1 bg-zinc-50 hover:bg-zinc-100 rounded-sm border border-gray-300 w-full divide-x divide-gray-800">
          <Image
        ...
        <Link href={`${process.env.PROD_BASE_URL}/api/auth/github`} className="py-3 px-6 gap-4 flex flex-1 justify-center rounded-sm border border-gray-300 w-full divide-x divide-gray-800">
          <Image
