...
export default function Page() {
  ...
  const handleRegister = async () => {
    ...
    try {
      const response = await fetch(`${process.env.PROD_BASE_URL}/api/auth/register`, {
        method: "POST",
        headers: {
      ...
      {isLoading ? (
        ...
        >
          ...
          <div className="w-full flex gap-4 flex-1">
            <Link
              href={`${process.env.PROD_BASE_URL}/api/auth/google`}
            ...
            <Link
              href={`${process.env.PROD_BASE_URL}/api/auth/github`}
