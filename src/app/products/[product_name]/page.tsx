...
type tab = "all" | "video" | "text";

export default function Page() {
  const pathname = usePathname();
  const space_name = pathname.split("/").filter(Boolean).pop() as string;

  const [nav, setnav] = useState("");
  const [errorMessage, setErrorMessage] = useState("");
  const inboxtab = sessionStorage.getItem("inbox-tab") as tab;
  const [tab, setTab] = useState<tab>(inboxtab || "all");
  useEffect(() => {
  ...
  const {
    ...
    async () => {
      try {
        const data = await fetchSpaceData(space_name);
        return data;
      } catch (error) {
  ...
  const fetchTestimonials = async (): Promise<TestimonialProps[]> => {
    if (!space_name) {
      return []; // Return an empty array if space_name is undefined
    }

    const tabcase = tab || sessionStorage.getItem("tab-index");

    switch (tabcase) {
      case "all":
        return await getAllTestimonials(space_name);
      case "video":
        return await getAllVideoTestimonials(space_name);
      case "text":
        return await getAlltextTestimonials(space_name);
      default:
        return []; // Return an empty array for any unrecognized tab
    }
  };
  ...
  const {
    ...
    error: isTetimonialsError,
  } = useQuery<TestimonialProps[]>(["testimonials", tab], fetchTestimonials, {
  ...
  useEffect(() => {
    if (tab) {
      queryClient.invalidateQueries(["testimonials", tab]); // Invalidate the query to refetch data
    }
  }, [tab]);
        ...
        <>
          ...
          <MaxWidthWrapper className="py-8 grid grid-cols-12 grid-flow-row w-full h-full min-h-64">
              ...
              <div className="space-y-2 w-full">
                ...
                <ul className="space-y-1">
                  <button
                    ...
                    className="flex items-center gap-4 py-1.5 px-2.5 hover:bg-gray-800 rounded-md text-base text-zinc-100 w-full"
                  >
            ...
            <div className="col-span-9">
              {isTestimonialsLoading ? (
              ...
              ) : testimonials && testimonials.length > 0 ? ( // Corrected the syntax here
                ...
                <div className="flex flex-col items-center gap-4 mx-auto">
                  ...
                  <h4 className="text-xl text-zinc-100">No testimonials yet</h4>
